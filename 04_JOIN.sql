...
-- 오라클 (JOIN이라는 단어를 작성하지 않음) -- 해석 할 때 난해한 부분이 있음
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE
...
/* 3 */SELECT EMP_ID,  EMP_NAME, EMP_NO
/*1-1*/FROM EMPLOYEE
...
SELECT EMP_NAME, E .SAL_LEVEL , MIN_SAL, S.SAL_LEVEL, MAX_SAL
FROM EMPLOYEE E
...
-- JOB.JOB_CODE			 (CHAR(2))
	--> 컬럼명, 자료형이 모드 일치! == NATURAL JOIN 대상
...
-- EMPLPYEE, DEPARTMENT, LOCATION 3개 조인
...
SELECT EMP_NAME 사원 , EMP_NO 주민번호 , DEPT_TITLE 부서명 , JOB_NAME 직급명
FROM JOB
JOIN EMPLOYEE USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME, SALARY , (SALARY * 12) + (SALARY * 12) * NVL(BONUS, 1)  연봉
FROM JOB 
JOIN EMPLOYEE USING (JOB_CODE)
JOIN SAL_GRADE USING (SAL_LEVEL)
WHERE SALARY > MIN_SAL;
/*GROUP BY SAL_LEVEL 
HAVING SAL_LEVEL > MIN_SAL ;*/

...
-- 컬럼 값: 행과 열이 교차된 테이블의 한 칸에 작성된 값

-- SELECT문 작성 시
-- 컬럼명에 산술 연산을 직접 작성하면
-- 조회 결과(RESULT_SET)에 연산 결과가 반영되어 조회된다
...
-- SYSDATE 			: 현재 시간 (년, 원, 일, 시, 분, 초) 조회
-- SYSTIMESTAMP : 현재 시간(년, 원, 일, 시, 분, 초, ms + 지역(local)) 조회
...
-- CEIL(숫자) : 올림 처리
-- 현재 시간 - 생년월일
SELECT CEIL((SYSDATE -TO_DATE('2003-06-01')) / 365)
FROM DUAL;
...
SELECT CEIL((SYSDATE -TO_DATE('2003-06-01')) / 365) AS 나이
FROM DUAL;
...
-- EMPOLYEE 테이블에서 DEPT_CODE가 없는 사원 조회
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE IS NULL; -- 2행

-- EMPOLYEE 테이블에서 DEPT_CODE가 있는 사원 조회
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
-- 오름차순 정렬 + NULLS FIRST (NULL인 경우 제일 위에)
SELECT EMP_NAME, PHONE 
FROM EMPLOYEE
ORDER BY PHONE NULLS FIRST;

-- 오름차순 정렬 + NULLS LAST (NULL인 경우 제일 아래)
SELECT EMP_NAME, PHONE 
FROM EMPLOYEE
ORDER BY PHONE /*NULLS LAST*/; -- 기본 값

-- 내림차순 정렬 + NULLS FIRST (NULL인 경우 제일 위에)
SELECT EMP_NAME, PHONE 
FROM EMPLOYEE
...
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
ORDER BY DEPT_CODE, SALARY DESC; -- DEPT_CODE 안에서 급여가 내림차순으로 정렬됨

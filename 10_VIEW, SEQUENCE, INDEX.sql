 ...
 *    NOFORCE(기본값): 서브쿼리에 사용된 테이블이 존재해야만 뷰 생성 (-- DB 이관 시 사용)
 *    
...
-- DEPT_COPY2 테이블에 DEPT_ID, LOCATION_ID 컬럼만을 이용해서
-- V_DCOPY2 VIEW 생성
...
SELECT * FROM DEPT_COPY2;
--> 원본에 값이 삽입 되었음을 확인!!
-- (가상 테이블 VIEW는 값 저장/수정/삭제 불가)
--> (그래서 연결된 원본이 변한다!!)

-- D0 / NULL / L2
--> NULL은 DB 무결성을 약하게 만드는 원인
---> 가능하면 NULL이 생기지 않게 하는 것이 좋다!!

-- WITH READ ONLY 옵션을 추가해서 읽기 전용으로 변겅!!
...
INCREMENT BY 5 -- NEXTVAR 호출 마다 5씩 증가
...
-- NEXTVAL 호출 시 마다 INCREMENT BY에 작성된 값 만큼 증가
...
--> 시퀀스는 MAXVLAUE 값을 초과해서 증가할 수 없다

...
-- EMPLOYEE 테이블에서 부서코드, 부서(그룹) 별 급여 합계 조회
		...
		/* 묶인 그룹 이름 */
SELECT DEPT_CODE, SUM(SALARY) -- 그룹별로 그룹함수(SUM) 연신 수행
FROM EMPLOYEE
...
-- 부서 코드 오름차순으로 정렬
SELECT DEPT_CODE 직급 , JOB_CODE 부서 ,  SUM(SALARY) "급여 합계"
FROM EMPLOYEE
...
--------------------------------------------------------------------------------------------------------------------------
-- WHERE 절 : 지정된 테이블에서 어떤 행만 조회 결과호 삼을건지
--					 조건을 지정하는 구문
...
SELECT DEPT_CODE "부서 코드 ", SUM(SALARY) 합계
FROM EMPLOYEE
GROUP BY DEPT_CODE 
...
-- 직급코드 오름차순으로 정렬하세요
SELECT JOB_CODE , COUNT(BONUS) 보너스
FROM EMPLOYEE
GROUP BY JOB_CODE 
ORDER BY JOB_CODE;
...
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 1,1 ) ='7' -- 주민등록번호 앞자리만 잘라서 봤을 때 첫번째 자리가 '7'이면 70년대생
GROUP BY DEPT_CODE
HAVING  FLOOR(AVG(SALARY)) >= 3000000
ORDER BY DEPT_CODE;
--------------------------------------------------------------------------------------------------------------                     
